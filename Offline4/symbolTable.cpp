...
public:
    ...
    bool insert(string name, string type, int size, vector<pair<string, string>> v, int sp)
    {
...
public:
    ...
    bool insert(string name, string type, int size = -1, vector<pair<string, string>> v = vector<pair<string, string>>(), int sp = -1)
    {
        if (cur_scope != NULL)
            return cur_scope->insert(name, type, size, v, sp);
        else
        {
            id.clear();
            id.pb(1);
            ...
            close = false;
            cur_scope->insert(name, type, size, v, sp);
        }
    }
